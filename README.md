...
- 下载+解包最新上游：`pnpm upstream:sync`（输出到 `.cache/upstream/unpacked/latest/`）
...
- get-models 实测（输出 feature_flags 报告）：`pnpm check:get-models:acemcp`

报告输出（不进 git）：
- `.cache/reports/upstream-endpoints.<version>.json`
...
- `pnpm check:matrix -- --unpack-dir .cache/upstream/unpacked/latest`

## 配置（BYOK Panel + 安全存储）

本项目新增 BYOK Providers 面板（Webview），用于更安全地管理 BYOK 配置：
- 非敏感（providers/routing/defaults/baseUrl）存 `globalState`
