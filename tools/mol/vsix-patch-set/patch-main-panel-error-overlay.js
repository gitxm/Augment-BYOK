...
function patchMainPanelErrorOverlay(filePath) {
  ...
  const injection =
    ...
    `window.addEventListener(\"error\",(e)=>{try{const err=e&&e.error?e.error:null;const stack=err&&err.stack?err.stack:\"\";const msg=stack||((e&&e.message)?e.message:String(e));` +
    `console.error(\"[Augment BYOK] webview error\",e);show(\"window.error\",msg);}catch{}});` +
    `window.addEventListener(\"unhandledrejection\",(e)=>{try{const r=e&&e.reason!==void 0?e.reason:e;const msg=r&&r.stack?r.stack:typeof r==\"string\"?r:JSON.stringify(r,null,2);` +
